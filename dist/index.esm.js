...
function useEmotion(options = {}) {
    ...
    // 상태
    const [state, setState] = useState(emotionEngine$2.getState());
    const [behaviorMetrics, setBehaviorMetrics] = useState(emotionEngine$2.getBehaviorMetrics());
    // 구독
    useEffect(() => {
        if (!autoSubscribe)
            return;
    ...
    // 상품 점수
    const getProductScore = useCallback((productId) => {
        return emotionEngine$2.getProductScore(productId);
    }, []);
    const getProductScores = useCallback((productIds) => {
    ...
    // 파생 값
    const theme = useMemo(() => emotionEngine$2.getTheme(), [state.emotion]);
...
function useEmotionTheme() {
    const [theme, setTheme] = useState(emotionEngine$2.getTheme());
    useEffect(() => {
...
const EmotionPulse = ({ emotion = 'neutral', confidence = 0.5, source = 'auto', size = 'md', position = 'bottom-right', showConfidence = true, onClick, onExpand, onSelectEmotion, className = '', animated = true }) => {
    ...
    const handleEmotionSelect = useCallback((selectedEmotion) => {
        if (onSelectEmotion) {
...
const EmotionFeedback = ({ enabled = true, maxMessages = 3, displayDuration = 3000, position = 'bottom-left', isNewUser = false, className = '' }) => {
    ...
    // 메시지 추가
    const addMessage = useCallback((feedback) => {
        if (!shouldShow)
            return;
        const newMessage = {
            ...feedback,
...
function useFeedback() {
    const addFeedback = useCallback((type, icon, message, detail, emotion) => {
...
const EmotionDemo = () => {
    ...
    // 수동 감정 선택
    const handleManualSelect = useCallback((emotion) => {
        setPrevEmotion(currentEmotion);
...
export { BehaviorTracker, BenchmarkEngine, EMOTION_ICONS, EMOTION_LABELS_EN, EMOTION_LABELS_KO, EMOTION_THEMES, EmotionBadge, EmotionBadgeCompact, EmotionDemo, EmotionEngine, EmotionFeedback, EmotionMatchIndicator, EmotionOverlay, EmotionPulse, EmotionTransitionToast, Hysteresis, MultimodalFusion, SDK_DESCRIPTION, SDK_NAME, SDK_VERSION, THEME_TRANSITION_CSS, TextAnalyzer, emotionEngine$1 as default, emotionEngine$2 as emotionEngine, getTheme, getThemeCSSVariables, printSDKInfo, useEmotion, useEmotionInit, useEmotionTheme, useFeedback, useProductEmotionScore };
